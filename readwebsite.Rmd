...
j1<-27
j2<-27 #range of journals to read   
...
print(myr)
  ...
  vol<-index$lastvol[j]
  startissue<-index$lastissue[j]
  #startissue<-5

  for (issue in seq(startissue,1,-1)){ #go backwards through issues
    ...
    for (n in n1:n2){
      ...
      #get the address for author
          ### #Need the html code for this author to get the address. This is sometimes missing, so put x
      #(for journals before 35, the affiliation is carried over from prior article)
        authorlist<-mya %>%
      html_elements("a")%>%
      html_attr("href")
     ...
     #remove /n
      ...
      articledf$jnumber[r]<-j
      
    }
  }
}  
  myfilename<-paste0('PSarticles_all.csv')
...
```{r findaffil}
affil<-'UK'
mydf<-read.csv('PSarticles_all.csv')
myaffils<-mydf$affiliation
g<-grep(affil,myaffils)
mysubset<-mydf[g,]
...
mydf<-read.csv('PSarticles_all.csv')
mydf$instemail<-1 #assume institutional

mye<-mydf$email.in.pdf
elist<-c('yahoo','.com','gmail','hotmail','outlook','live.')
...
mydf$instemail[g]<-0
 ...
 myfilename<-paste0('PSarticles_all.csv')
  ...
  write.csv(mydf,myfilename,row.names=F)
```